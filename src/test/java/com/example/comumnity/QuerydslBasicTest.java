...
import com.example.comumnity.domain.test.dto.MemberDto;
import com.example.comumnity.domain.test.dto.QMemberDto;
import com.example.comumnity.domain.test.dto.UserDto;
import com.example.comumnity.domain.test.entity.Member;
import com.example.comumnity.domain.test.entity.QMember;
import com.example.comumnity.domain.test.entity.QTeam;
import com.example.comumnity.domain.test.entity.Team;
...
import static com.example.comumnity.domain.test.entity.QMember.*;
import static com.example.comumnity.domain.test.entity.QTeam.team;
...
//        em.createQuery("select m from Member m", MemberDto.class); // -> 이렇게 조회하면 안됨
        List<MemberDto> resultList = em.createQuery("select new com.example.comumnity.domain.test.dto.MemberDto(m.userName, m.age)  from Member m", MemberDto.class)
                .getResultList();
    ...
    void findDtoByQueryProjection() {
        ...
        List<MemberDto> result = queryFactory
                .select(new QMemberDto(member.userName, member.age)) // 생성자 방식과 차이점은 생성자 방식은 런타임 오류 : 실행을 해야만 오류 쿼리 프로젝션 : 컴파일 오류 실행 전에 오류를 잡아줌. -> DTO 생성자에 @QueryProjection 붙여주고 사용
